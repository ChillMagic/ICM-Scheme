...
(library (Vector)
         (export new make is? length get set for-each for-each-with-inter)
...
(library (HashTable)
         (export new is? size entries keys values insert! get set get! for-each for-each-with-inter)
         (import
...
(library (ToStringFormat)
         ...
         (define (display-to-string v)
           (let-values (((port result) (open-string-output-port)))
             ...
             (result)))
         (define (to-pformat e)
           ((Vector.get (HashTable.get SystemToStringFormatTable (type e)) 1) e))
         (define (to-string e)
           ((Vector.get (HashTable.get SystemToStringFormatTable (type e)) 0) e)))
