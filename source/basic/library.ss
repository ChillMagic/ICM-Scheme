...
(library (Vector)
  (export new make is? length
          get set for-each for-each-with-inter symbol-sort)
  (import (except (rnrs) + length for-each)
          (prefix (rnrs) rnrs.))
...
(library (Stack)
  ...
  (define (set-dat stack dat)
    (vector-set! stack 1 dat))
  (define (top stack)
    (car (get-dat stack)))
  (define (push stack e)
    (vector 'stack (append (list e) (get-dat stack))))
  (define (push! stack e)
    (let ((r (append (list e) (get-dat stack))))
      (set-dat stack r) r))
  (define (pop stack)
    (vector 'stack (cdr (get-dat stack))))
  (define (pop! stack)
    (let ((r (top stack)))
...
(library (ToStringFormat)
  (export to-string to-pformat
          type display-to-string SystemToStringFormatTable)
  (import (rnrs)
          (prefix (Vector) Vector.)
  ...
  (define (to-pformat e)
    ((Vector.get
      (HashTable.get SystemToStringFormatTable (type e) (unfind e)) 1) e))
  (define (to-string e)
    ((Vector.get
      (HashTable.get SystemToStringFormatTable (type e) (unfind e)) 0) e))
  (define (unfind e)
    (display "Unfind ")
    (write e)
    (display ".\n")))
